...
top5novo <- atividade %>% group_by(partido, UF) %>%  summarise(contagem = n()) %>% arrange(-contagem) %>% top_n(5)

atividade %>% group_by(UF,partido) %>%  summarise(contagem = n()) %>% arrange(-contagem) %>% top_n(5)

atividade %>% 
  filter(partido %in%  (top5novo %>% pull(partido))) %>% 
  ggplot(aes(partido, UF, label = twitter, color = partido)) +
  geom_text(check_overlap = TRUE)
